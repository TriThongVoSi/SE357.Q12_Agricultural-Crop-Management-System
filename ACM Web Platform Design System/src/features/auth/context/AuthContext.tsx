...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const refreshUserFromToken = useCallback(async () => {
    try {
      ...
      const primaryRole = (result.role?.toLowerCase() || result.roles?.[0]?.toLowerCase()) as UserRole;
  ...
  const login = async (
    ...
    try {
      ...
      const primaryRole = (role?.toLowerCase() || roles[0]?.toLowerCase()) as UserRole;
      ...
      const normalizedRedirect =
        redirectTo && redirectTo !== '/' ? redirectTo : `/${primaryRole}`;
