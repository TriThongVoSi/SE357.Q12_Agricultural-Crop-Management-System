...
public class FarmerOwnershipService {
    ...
    /**
     * Check if current user is an admin (bypasses ownership checks).
     * 
    ...
    /**
     * Require owned farm, but allow admin bypass.
     * Admin users can access any farm.
     * 
     * @param farmId the farm ID to check
     * @return the Farm entity
     */
    public Farm requireOwnedFarmOrAdmin(Integer farmId) {
        if (isCurrentUserAdmin()) {
            return farmRepository.findById(farmId)
                    .orElseThrow(() -> new AppException(ErrorCode.FARM_NOT_FOUND));
        }
        return requireOwnedFarm(farmId);
    }

    /**
     * Require owned plot, but allow admin bypass.
     * Admin users can access any plot.
     * 
     * @param plotId the plot ID to check
     * @return the Plot entity
     */
    public Plot requireOwnedPlotOrAdmin(Integer plotId) {
        if (isCurrentUserAdmin()) {
            return plotRepository.findById(plotId)
                    .orElseThrow(() -> new AppException(ErrorCode.PLOT_NOT_FOUND));
        }
        return requireOwnedPlot(plotId);
    }

    /**
     * Require owned season, but allow admin bypass.
     * Admin users can access any season.
     * 
     * @param seasonId the season ID to check
     * @return the Season entity
     */
    public Season requireOwnedSeasonOrAdmin(Integer seasonId) {
        if (isCurrentUserAdmin()) {
            return seasonRepository.findById(seasonId)
                    .orElseThrow(() -> new AppException(ErrorCode.SEASON_NOT_FOUND));
        }
        return requireOwnedSeason(seasonId);
    }
}
