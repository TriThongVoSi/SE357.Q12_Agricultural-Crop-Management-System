...
public class FarmerOwnershipServiceTest {
    ...
    @Test
    @DisplayName("requireOwnedFarmOrAdmin allows admin to access any farm")
    void testAdminBypass_Farm() {
        when(currentUserService.isAdmin()).thenReturn(true);
        when(farmRepository.findById(foreignFarm.getId())).thenReturn(Optional.of(foreignFarm));

        Farm result = ownershipService.requireOwnedFarmOrAdmin(foreignFarm.getId());

        assertNotNull(result);
        assertEquals(foreignFarm.getId(), result.getId());
    }

    @Test
    @DisplayName("requireOwnedPlotOrAdmin allows admin to access any plot")
    void testAdminBypass_Plot() {
        when(currentUserService.isAdmin()).thenReturn(true);
        when(plotRepository.findById(foreignPlot.getId())).thenReturn(Optional.of(foreignPlot));

        Plot result = ownershipService.requireOwnedPlotOrAdmin(foreignPlot.getId());

        assertNotNull(result);
        assertEquals(foreignPlot.getId(), result.getId());
    }

    @Test
    @DisplayName("requireOwnedSeasonOrAdmin allows admin to access any season")
    void testAdminBypass_Season() {
        when(currentUserService.isAdmin()).thenReturn(true);
        when(seasonRepository.findById(foreignSeason.getId())).thenReturn(Optional.of(foreignSeason));

        Season result = ownershipService.requireOwnedSeasonOrAdmin(foreignSeason.getId());

        assertNotNull(result);
        assertEquals(foreignSeason.getId(), result.getId());
    }
